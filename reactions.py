...
async def on_raw_reaction_add(payload):
    ...
    def cancel_proposal(proposal, reason, voting_message) {
        ...
        if reason == ProposalResult.CANCELLED_BY_PROPOSER:
            response_to_proposer = PROPOSAL_RESULT_PROPOSER_RESPONSE[reason].format(author=mention_author)
            edit_in_voting_channel = PROPOSAL_RESULT_VOTING_CHANNEL[reason].format()
            log_message = "(by the proposer)"
        else if reason == ProposalResult.CANCELLED_BY_REACHING_THRESHOLD:
            response_to_proposer = PROPOSAL_RESULT_PROPOSER_RESPONSE[reason].format(author=mention_author, threshold=LAZY_CONSENSUS_THRESHOLD, voting_link=link_to_voting_message)
            edit_in_voting_channel = PROPOSAL_RESULT_VOTING_CHANNEL[reason].format(threshold=LAZY_CONSENSUS_THRESHOLD, list_of_voters)
    ...
    if proposal.author_id == payload.user_id:
        cancel_proposal(proposal, ProposalResult.CANCELLED_BY_PROPOSER, voting_message)
        return
    ...
    if len(proposal.voters) < LAZY_CONSENSUS_THRESHOLD:
        ...
        cancel_proposal(proposal, ProposalResult.CANCELLED_BY_REACHING_THRESHOLD, voting_message)
        return
